...
for input_file in *.out; do
    ...
    #overlap_value=$(grep -m 2 $nroots "excitation energy (eV) =" "$input_file" | cut -d "=" -f2)
    overlap_value=$(grep "excitation energy (eV) =" "$input_file" | cut -d ":" -f2)
